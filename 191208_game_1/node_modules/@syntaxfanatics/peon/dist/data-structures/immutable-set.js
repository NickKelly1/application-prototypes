...
class ImmutableSet {
    ...
    *[Symbol.iterator]() {
     ...
     */
    add(...items) {
        const newImmutableSet = new ImmutableSet(Array.from(this.inner).concat(items));
    ...
    concat(...iterables) {
        ...
        const newImmutableSet = new ImmutableSet(newValue);
        return newImmutableSet;
    }
    /**
     * @description
    ...
    filter(predicate) {
        const newSet = new Set();
        this.forEach(item => { if (predicate(item))
            newSet.add(item); });
        return new ImmutableSet(newSet);
    }
}
